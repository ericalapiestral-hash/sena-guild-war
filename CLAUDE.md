...
- **보안 — 진짜 문은 워커 하나뿐이다**(2026-09-08 전면 점검). 사이트는 정적 번들이라 통째로 공개된다. 프론트의 검사는 전부 '화면을 어디로 보낼까'용이다.
  ...
  - **회귀 테스트: `worker/sec-test.mjs`.** `npx wrangler dev --local --port 8799 --var ADMIN_PW:testpw` 를 띄우고 `node worker/sec-test.mjs <실행이름>`. 로컬 KV 가 실행 사이에 남으므로 매번 다른 실행이름을 줄 것.
