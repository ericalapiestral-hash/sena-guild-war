...
학습 자동 루틴(cron) 도입 → AI 검색 잔재 정리 + 학습 브리핑 [데이터]로 복귀 → 소탕 버튼 제거 → 브라우저 밖 push 차단 → 커트라인 기준 메뉴 → 덱 추출 중복 제거 → 학습 성능 개선(모델 상향 + 전면 병렬화) → 학습 v2(스크린샷 비전 분석) → 학습 브리핑 홈에서 내림 → 학습 API(/learn) 도입 → 캡처 여러 장 이어 붙이기
